...
class CalcRate(object):
    """Class object to calculate corona virus death and recovery rate in %. """

    def __init__(self, num_cases, num_deaths, num_recover=None):
        self.num_cases = num_cases
        self.num_deaths = num_deaths
        self.num_recover = num_recover

    def death(self):
        """Function returns virus death rate. """
        try:
    ...
    def recover(self):
        """Function returns virus recovery rate """
        return((self.num_recover / self.num_cases) * 100)
...
def setup_logging_Enhanced(file):
    ...
    create_log_file('logs/' + TimeStamp + components +
                    '_DEBUG.log', logging.DEBUG)
    create_log_file('logs/' + TimeStamp + components +
                    '_INFO.log', logging.INFO)
    create_log_file('logs/' + TimeStamp + components +
                    '_WARNING.log', logging.WARNING)
    create_log_file('logs/' + TimeStamp + components +
                    '_ERROR.log', logging.ERROR)
